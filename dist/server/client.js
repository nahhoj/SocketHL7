...
const sendMessageSocket = (req) => {
    ...
    count = 0;
    request = req;
    for (let x = 0, y = 0; x < req.start.length; x += 2, y++) {
        start += String.fromCharCode(utils_1.convertAscii(req.start.substring(x, x + 2)));
    }
    for (let x = 0, y = 0; x < req.end.length; x += 2, y++) {
        end += String.fromCharCode(utils_1.convertAscii(req.end.substring(x, x + 2)));
    }
    if (req.hl7) {
        let c = 0;
        let l = [];
